  ...
  <div>
    ...
    <div v-if="isOrderCompositionModalOpen" class="modal">
      <div class="modal-content">
        ...
        <ul>
          <li
            v-for="(item, itemIndex) in newOrders[selectedOrderIndex].composition"
            :key="itemIndex"
          >
            {{ item }}
          </li>
...
export default {
  ...
  methods: {
    ...
    fetchNewOrders() {
      axios
        .get('http://185.128.106.222:3000/delivery/all', {
          headers: {
            Authorization:
        ...
        .then((response) => {
          this.newOrders = response.data
            ...
            .map((order) => ({
              customerName: order.client ? order.client.fullName : 'Неизвестный клиент',
          ...
          this.orderHistory = response.data
            ...
            .map((order) => ({
              ...
              executionTime: order.executionTime
                ? new Date(order.executionTime).toLocaleTimeString()
                : 'Неизвестно'
            }))
        })
        .catch((error) => {
    ...
    fetchCouriers() {
      axios
        .get('http://185.128.106.222:3000/auth/staff', {
          headers: {
            Authorization:
    ...
    assignOrder() {
      ...
      const status =
        this.selectedStatus === 'Доставлен'
          ? 3 // 3 - статус "Доставлен"
          : this.selectedStatus === 'Передан курьеру'
            ? 2 // 2 - статус "Передан курьеру"
    ...
    updateOrderStatusBackend(order) {
      if (order.status === 'Передан курьеру') order.status = 2
      if (order.status === 'Доставлен') order.status = 3

      const data = {
        id: order.orderNumber, // Замените это на фактическое имя свойства для идентификатора заказа в вашем бекенде
        status: order.status,
        courier: {
          id: order.courier ? order.courier.id : null // Замените это на фактическое имя свойства для идентификатора курьера в вашем бекенде
        }
      }

      axios
        .patch(`http://185.128.106.222:3000/delivery/`, data, {
          headers: {
            'Content-Type': 'application/json',
            Authorization:
              'Bearer eyJhbGciOiJIUzI1NiJ9.eyJyb2xlIjoxfQ.tXLyzqivhPuBcehRerTCZj_gkXGb5NAASDcP0nZr5hc'
