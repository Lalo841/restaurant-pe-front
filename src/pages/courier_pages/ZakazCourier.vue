  ...
  <div>
    ...
    <div>
      ...
      <div v-for="(order, index) in newOrders" :key="index" class="record">
        ...
        <button @click="acceptOrderByCourier(index)" class="manage-btn">Принять заказ</button>
      </div>
    </div>
    ...
    <div>
      ...
      <div v-for="(order, index) in currentOrders" :key="index" class="record">
        ...
        <button @click="completeOrderByCourier(index)" class="manage-btn">Доставлен</button>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    // Принять заказ
    acceptOrderByCourier(index) {
      const selectedOrder = this.newOrders[index];
      // Проверяем, совпадает ли id курьера с id курьера у выбранного заказа
      if (selectedOrder.courier.id === this.selectedCourier) {
        // Меняем статус заказа на "Принят"
        selectedOrder.status = 2;
        // Перемещаем заказ из newOrders в currentOrders
        this.currentOrders.push(selectedOrder);
        this.newOrders.splice(index, 1);
      } else {
        // Обрабатываем случай, когда курьер не назначен для этого заказа
        console.error("Курьер не назначен для этого заказа.");
      }
    },
    ...
    completeOrderByCourier(index) {
      ...
      if (selectedOrder.courier.id === this.selectedCourier) {
        ...
        selectedOrder.executionTime = new Date().toLocaleTimeString();
        // Перемещаем заказ из currentOrders в orderHistory
        this.orderHistory.push(selectedOrder);
        this.currentOrders.splice(index, 1);
      } else {
        // Обрабатываем случай, когда курьер не назначен для этого заказа
