...
const openModal = (record) => {
  selectedRecord.value = record
    ? { ...record, tablesInfo: record.tables?.map((table) => table.capacity).join(' ') }
    : { tablesInfo: '' }
      ...
      <div class="modal" style="background-color: #c4a189">
        ...
        <div class="mb-4">
          <label class="block text-sm font-medium text-gray-700"
            >Столики (вместимость через пробел)</label
          >
          <input v-model="selectedRecord.tablesInfo" class="input-box" />
        </div>
        <button @click="editRecord" class="add-button red-button">
          {{ selectedRecord.id ? 'Изменить' : 'Добавить' }}
        </button>
        <button @click="deleteRecord" class="add-button red-button" v-if="selectedRecord.id">
          Удалить
        </button>
